...
df = pd.read_csv('data/Minyak Goreng Curah.csv')
...
print("Dataframe Normal (Minyak Goreng Curah) setelah mengatasi missing values:")
df.head()
...
exclude_columns = ['Minyak Goreng Curah', 'Date', 'ddd_car']
...
print("Dataframe Normal (Minyak Goreng Curah) setelah one-hot encoding:")
df.head(10)
...
fitur = df.drop(columns=['Minyak Goreng Curah', 'Date']).columns.tolist()

# Pemisahan fitur dan target
X_train = split_data_sets['train'][fitur].values
y_train = split_data_sets['train']['Minyak Goreng Curah'].values
X_val = split_data_sets['val'][fitur].values
y_val = split_data_sets['val']['Minyak Goreng Curah'].values
X_test = split_data_sets['test'][fitur].values
y_test = split_data_sets['test']['Minyak Goreng Curah'].values
...
model.save_weights('minyak_goreng_curah.weights.h5')