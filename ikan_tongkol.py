...
df = pd.read_csv('data/Ikan Tongkol.csv')
...
print("Dataframe Normal (Ikan Tongkol) setelah mengatasi missing values:")
df.head()
...
exclude_columns = ['Ikan Tongkol', 'Date', 'ddd_car']
...
print("Dataframe Normal (Ikan Tongkol) setelah one-hot encoding:")
df.head(10)
...
fitur = df.drop(columns=['Ikan Tongkol', 'Date']).columns.tolist()

# Pemisahan fitur dan target
X_train = split_data_sets['train'][fitur].values
y_train = split_data_sets['train']['Ikan Tongkol'].values
X_val = split_data_sets['val'][fitur].values
y_val = split_data_sets['val']['Ikan Tongkol'].values
X_test = split_data_sets['test'][fitur].values
y_test = split_data_sets['test']['Ikan Tongkol'].values
...
model.save_weights('ikan_tongkol.weights.h5')