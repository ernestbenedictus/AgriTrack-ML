...
def normalize_data(df, exclude_columns):
    ...
    scaled_target = target_scaler.fit_transform(df[['Tepung Terigu Kemasan']])

    scaled_df = pd.DataFrame(scaled_features, columns=features.columns)
    scaled_df['Tepung Terigu Kemasan'] = scaled_target
...
def main():
    df = pd.read_csv('data/Tepung Terigu Kemasan.csv')
    ...
    exclude_columns = ['Tepung Terigu Kemasan', 'Date', 'ddd_car']
    ...
    fitur = df.drop(columns=['Tepung Terigu Kemasan', 'Date']).columns.tolist()

    X_train = train[fitur].values
    y_train = train['Tepung Terigu Kemasan'].values
    X_val = val[fitur].values
    y_val = val['Tepung Terigu Kemasan'].values
    X_test = test[fitur].values
    y_test = test['Tepung Terigu Kemasan'].values
...
if __name__ == '__main__':
    model = main()
    model.save_weights('tepung_terigu_kemasan.weights.h5')
