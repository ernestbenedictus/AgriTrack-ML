...
def normalize_data(df, exclude_columns):
    ...
    scaled_target = target_scaler.fit_transform(df[['Telur Ayam Ras']])

    scaled_df = pd.DataFrame(scaled_features, columns=features.columns)
    scaled_df['Telur Ayam Ras'] = scaled_target
...
def main():
    df = pd.read_csv('data/Telur Ayam Ras.csv')
    ...
    exclude_columns = ['Telur Ayam Ras', 'Date', 'ddd_car']
    ...
    fitur = df.drop(columns=['Telur Ayam Ras', 'Date']).columns.tolist()

    X_train = train[fitur].values
    y_train = train['Telur Ayam Ras'].values
    X_val = val[fitur].values
    y_val = val['Telur Ayam Ras'].values
    X_test = test[fitur].values
    y_test = test['Telur Ayam Ras'].values
...
if __name__ == '__main__':
    model = main()
    model.save_weights('telur_ayam.weights.h5')
