...
def normalize_data(df, exclude_columns):
    ...
    scaled_target = target_scaler.fit_transform(df[['Minyak Goreng Kemasan Sederhana']])

    scaled_df = pd.DataFrame(scaled_features, columns=features.columns)
    scaled_df['Minyak Goreng Kemasan Sederhana'] = scaled_target
...
def main():
    df = pd.read_csv('data/Minyak Goreng Sederhana.csv')
    ...
    exclude_columns = ['Minyak Goreng Kemasan Sederhana', 'Date', 'ddd_car']
    ...
    fitur = df.drop(columns=['Minyak Goreng Kemasan Sederhana', 'Date']).columns.tolist()

    X_train = train[fitur].values
    y_train = train['Minyak Goreng Kemasan Sederhana'].values
    X_val = val[fitur].values
    y_val = val['Minyak Goreng Kemasan Sederhana'].values
    X_test = test[fitur].values
    y_test = test['Minyak Goreng Kemasan Sederhana'].values
...
if __name__ == '__main__':
    ...
    model.save_weights("minyak_goreng_kemasan.weights.h5")
