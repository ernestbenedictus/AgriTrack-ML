...
def normalize_data(df, exclude_columns):
    ...
    scaled_target = target_scaler.fit_transform(df[['Beras Premium']])

    scaled_df = pd.DataFrame(scaled_features, columns=features.columns)
    scaled_df['Beras Premium'] = scaled_target
...
def main():
    df = pd.read_csv('data/Beras Premium.csv')
    ...
    exclude_columns = ['Beras Premium', 'Date', 'ddd_car']
    ...
    fitur = df.drop(columns=['Beras Premium', 'Date']).columns.tolist()

    X_train = train[fitur].values
    y_train = train['Beras Premium'].values
    X_val = val[fitur].values
    y_val = val['Beras Premium'].values
    X_test = test[fitur].values
    y_test = test['Beras Premium'].values
...
if __name__ == '__main__':
    model = main()
    model.save_weights('beras_premium.weights.h5')
