...
def normalize_data(df, exclude_columns):
    ...
    scaled_target = target_scaler.fit_transform(df[['Daging Ayam Ras']])

    scaled_df = pd.DataFrame(scaled_features, columns=features.columns)
    scaled_df['Daging Ayam Ras'] = scaled_target
...
def main():
    df = pd.read_csv('data/Daging Ayam Ras.csv')
    ...
    exclude_columns = ['Daging Ayam Ras', 'Date', 'ddd_car']
    ...
    fitur = df.drop(columns=['Daging Ayam Ras', 'Date']).columns.tolist()

    X_train = train[fitur].values
    y_train = train['Daging Ayam Ras'].values
    X_val = val[fitur].values
    y_val = val['Daging Ayam Ras'].values
    X_test = test[fitur].values
    y_test = test['Daging Ayam Ras'].values
...
if __name__ == '__main__':
    model = main()
    model.save_weights('ayam_ras.weights.h5')
