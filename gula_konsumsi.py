...
df = pd.read_csv('data/Gula Konsumsi.csv')
...
print("Dataframe Normal (Gula Konsumsi) setelah mengatasi missing values:")
df.head()
...
exclude_columns = ['Gula Konsumsi', 'Date', 'ddd_car']
...
print("Dataframe Normal (Gula Konsumsi) setelah one-hot encoding:")
df.head(10)
...
fitur = df.drop(columns=['Gula Konsumsi', 'Date']).columns.tolist()

# Pemisahan fitur dan target
X_train = split_data_sets['train'][fitur].values
y_train = split_data_sets['train']['Gula Konsumsi'].values
X_val = split_data_sets['val'][fitur].values
y_val = split_data_sets['val']['Gula Konsumsi'].values
X_test = split_data_sets['test'][fitur].values
y_test = split_data_sets['test']['Gula Konsumsi'].values
...
model.save_weights('gula_konsumsi.weights.h5')