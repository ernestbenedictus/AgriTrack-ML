...
df = pd.read_csv('data/Garam Halus Beryodium.csv')
...
print("Dataframe Normal (Garam Halus Beryodium) setelah mengatasi missing values:")
df.head()
...
exclude_columns = ['Garam Halus Beryodium', 'Date', 'ddd_car']
...
print("Dataframe Normal (Garam Halus Beryodium) setelah one-hot encoding:")
df.head(10)
...
fitur = df.drop(columns=['Garam Halus Beryodium', 'Date']).columns.tolist()

# Pemisahan fitur dan target
X_train = split_data_sets['train'][fitur].values
y_train = split_data_sets['train']['Garam Halus Beryodium'].values
X_val = split_data_sets['val'][fitur].values
y_val = split_data_sets['val']['Garam Halus Beryodium'].values
X_test = split_data_sets['test'][fitur].values
y_test = split_data_sets['test']['Garam Halus Beryodium'].values
...
model.save_weights('garam_halus_beryodium.weights.h5')